...
export default function AdminEditProductPage() {
      ...
      api.get<any>(`/products/${productId}`)
        .then(p => {
          setForm({
            ...
            price: p.price || { YER: 0, SAR: 0, USD: 0 },
            category: p.category?._id || p.category || '',
            variants: p.variants?.length ? p.variants : [{ size: 'M', color: { name: { ar: 'افتراضي', en: 'Default' }, hex: '#000000' }, stock: p.totalStock || 0 }],
  ...
  const handleSubmit = async () => {
    ...
    await api.put(`/products/${productId}`, data, token);
  ...
  const handleImageUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      for (let i = 0; i < files.length; i++) {
        ...
        // Ensure the returned URL has the correct hostname if relative
    ...
    <div className="page-enter">
      ...
      <div style={{ maxWidth: 800 }}>
        ...
        <div style={{ display: 'flex', gap: 12 }}>
          <button className="btn btn-primary btn-lg" onClick={handleSubmit}>{locale === 'ar' ? 'حفظ التعديلات' : 'Save Changes'}</button>
